# Apex Report - WNS / Scan #1 (zRouter findings only)

> **Note:** The following findings from the original report have been extracted to their respective audit directories as they do not pertain to zRouter:
> - **WNS-1** (ZAMM orderbook transient balance bypass) — moved to `audit/ZAMM/`
> - **WNS-14** (Dapp XSS: unescaped on-chain name) — moved to `audit/NameNFT/`
> - **WNS-13** (Commit-reveal front-running via router) — moved to `audit/NameNFT/` (NameNFT/dapp issue; zRouter patch included there)
> - **WNS-10** (NameNFT refund misdirection to router) — moved to `audit/NameNFT/` (NameNFT/dapp issue; dapp patch included there)
