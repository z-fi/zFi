...
/// @dev Architecture: the HTML payload (97818 B) is the runtime bytecode of
...
///      compatibility (ERC-4804). Splitting the page across 4 data contracts
///      means EIP-170 caps each chunk, not the dapp
///      (24576 B per chunk, 486 B headroom).
///
...
contract zSwap {
    ...
    /// @dev The HTML payload lives in four separate data contracts whose runtime
    ...
    constructor(address data1, address data2, address data3, address data4) {
        if (
            data1 == data2 || data1 == data3 || data1 == data4 || data2 == data3
                || data2 == data4 || data3 == data4 || data1.code.length == 0
                || data2.code.length == 0 || data3.code.length == 0 || data4.code.length == 0
        ) revert InvalidData();
        DATA1 = data1;
        DATA2 = data2;
        DATA3 = data3;
...
<div class="tabs"><button id="tabSwap" class="on">Swap</button><button id="tabSend">Send</button><button id="tabBook">Orders</button></div>
...
<div class="panel" id="rcvPanel"><div class="hdr"><small id="rcvHdr">You receive</small><span class="pill"><span id="toIcon"></span><select id="toSel"></select></span></div><div class="row"><input id="outAmt" type="text" inputmode="decimal" placeholder="0.0"></div><small id="rate"></small></div>
...
try{sell=parseUnits(amt.value.trim(),f.dec);want=parseUnits(outAmt.value.trim(),t.dec);
...
const SB2="0x0000006c0fBc8CBAe822c41C9DC00956D0941e23";
const SB1="0x000000fF3D7A2d373615141d7489Ca66683DbecF";
const SBVIEW="0x000000B95ee642F1A216ef85b54BF77C127b1F50";
const SWAPBOL="0x00000040Ba80f8dc500d10ea6cF889b518592756";
const DUTCH="0x000000b87444cAd0beb79545dcaE8b4508d48179";
const ORDERBOL="0x0000000B98f59027FAFEac53daEf59D1135e1502";
const SEL_NEXTID="2a58b330",SEL_GETORDERS="03652027",SEL_CANCELORD="514fcac7";
const SEL_FILL1="c37dfc5b",SEL_FILL2="8ab3bfc9",SEL_FILL2_UNWRAP="402ad677",SEL_FILL2_ETH="13092239";
...
const SEL_ORDER_FIXED="63b5a5be",SEL_ORDER_DUTCH="921f90cb",SEL_CHECKPOINT="a972985e";
const BOARDS=[{a:SB2,v2:1},{a:SB1,v2:0}];
const WORDS=b=>b?11:6;   // static struct width, so decoding is positional
let bookSeq=0,bookRows=[],candCache={};
...
async function leg(addr,amt,isNft,meta){
  ...
  return{ic:k?sm(k.icon):genIcon(m.sym),txt:`${trimAmt(amt,m.dec)} ${k?safeSym(k.sym):'“'+m.sym+'”'}`,ok:!!k,link:escan(addr)};
}
...
async function loadBook(){
  if(!account||tab!=="book"){book.innerHTML="";return}
  const my=++bookSeq;
  const live=[];
  ...
  bookRows=rows;
  if(!rows.length){book.innerHTML='<h6><span>Orderbook</span><span>0</span></h6>';return}
    ...
    const data=r.dutch
      ...
      :r.v2
        ?"0x"+(nativeIn?SEL_FILL2_ETH:nativeOut?SEL_FILL2_UNWRAP:SEL_FILL2)
          +encUint(id)+encUint(dl)+(nativeIn?"":encUint(0n))+encAddr(ZERO) // 0 = full fill
