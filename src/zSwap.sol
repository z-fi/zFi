...
/// @dev Architecture: the HTML payload (650049 B) is the runtime bytecode of
///      27 data contracts, deployed separately and passed to the constructor.
...
///      compatibility (ERC-4804). Splitting the page across 27 data contracts
///      means EIP-170 caps each chunk, not the dapp
///      (24576 B per chunk, 13503 B headroom).
///
...
contract zSwap {
    ...
    /// @dev The HTML payload lives in twenty-seven separate data contracts whose
    ...
    // ABOUT LINEAGE, never a redirect: this contract serves its own twenty-seven chunks
    ...
    /// @dev Reassembles the page from all twenty-seven chunks in one pass: each chunk
