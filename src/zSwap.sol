...
/// @dev Architecture: the HTML payload (49424 B) is the runtime bytecode of
///      3 data contracts, deployed separately and passed to the constructor.
...
///      compatibility (ERC-4804). Splitting the page across 3 data contracts
///      means EIP-170 caps each chunk, not the dapp
///      (24576 B per chunk, 24304 B headroom).
///
...
contract zSwap {
    ...
    /// @dev The HTML payload lives in three separate data contracts whose runtime
    ...
    /// @dev Reassembles the page from all three chunks in one pass: each chunk is
...
<style>
:root{--b:#fafafa;--c:#fff;--p:#f6f6f6;--f:#000;--m:#888;--n:#666;--e:#ddd;--k:#000;--kf:#fff;--kh:#333;--fh:#ebebeb;--s:0 1px 3px #0000000a,0 8px 24px #0000000f}
.d{--b:#101010;--c:#181818;--p:#242424;--f:#eee;--m:#999;--n:#999;--e:#3d3d3d;--k:#fff;--kf:#000;--kh:#ddd;--fh:#303030;--s:none}
...
.meta{display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:.5em;font-size:.85em;color:var(--n);margin-bottom:.75em}
...
@media(pointer:coarse){.rcpt,.meta input{font-size:16px}.p button,.tabs button{padding:.6em .8em}}
</style>
...
<div class="card">
<div class="meta"><span id="addr">Not connected</span><label id="slipL">Slippage <input id="slip" type="number" value="0.5" min="0.01" max="10" step="0.1">%</label><button id="lk" title="Copy link">⧉</button><button id="th" title="Theme">☾</button></div>
<div class="tabs"><button id="tabSwap" class="on">Swap</button><button id="tabSend">Send</button></div>
<div class="panel"><div class="hdr"><small id="payL">You pay</small><span class="pill"><span id="fromIcon"></span><select id="fromSel"></select></span></div><div class="row"><input id="amt" type="text" inputmode="decimal" placeholder="0.0"></div><small id="bal"></small></div>
<button id="flip" class="flip" title="Flip"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14m-5-5 5 5 5-5"/></svg></button>
<div class="panel" id="rcvPanel"><div class="hdr"><small>You receive</small><span class="pill"><span id="toIcon"></span><select id="toSel"></select></span></div><div class="row"><input id="outAmt" type="text" inputmode="decimal" placeholder="0.0"></div><small id="rate"></small></div>
...
const SEL_RPERMIT="7ac2ff7b",SEL_MULTICALL="ac9650d8";
...
const sig=strip0x(await rpc("eth_signTypedData_v4",[owner,JSON.stringify(td)]));
const n=sig.length/2;if(n!==64&&n!==65)throw Error("bad signature");
...
const k=c&&(c["0x1"]||c[1]||c["1"]);
return!!(k&&k.atomic&&(k.atomic.status==="supported"||k.atomic.status==="ready"))}catch{return false}};
...
let account=null,last=null,fromBalance=0n,mode="in",bSeq=0,tab="swap",sendReady=null,sSeq=0;
async function refreshBalance(){
if(!account){bal.textContent="";fromBalance=0n;render();return}
...
bal.innerHTML=`Balance: <a href="#">${pretty}</a>`;
...
const syncDisabled=()=>{
for(const opt of fromSel.options)opt.disabled=(opt.value===toSel.value&&opt.value!=="__custom");
for(const opt of toSel.options)opt.disabled=(opt.value===fromSel.value&&opt.value!=="__custom");
};
...
let rcv="";
if(rv){
if(/^0x[0-9a-fA-F]{40}$/.test(rv)){rcv=rv;rcvEl.textContent=""}
...
if(!rv&&((f.addr===ZERO&&t.addr===WETH)||(f.addr===WETH&&t.addr===ZERO))){
outEl.value=inEl.value;fitFont(outEl);
rate.textContent=`1 ${f.sym} = 1 ${t.sym}`;
last={callData:f.addr===ZERO?"0xd0e30db0":"0x2e1a7d4d"+encUint(swapAmt),msgValue:f.addr===ZERO?swapAmt:0n,amountIn:swapAmt,from:f,to:WETH,exp:Date.now()+1500000};
render();return;
}
...
const Q=(x,d,u,v,S,sb)=>rpc(C,[{to:ZQUOTER,data:"0x"+x+d},L]).then(z=>decQ(z,sb||slipBps,!isIn,u,v,S));
let r=null;
...
// every builder is quoted concurrently: these are heavy multi-pool calls and
// running them in series made a quote cost four round-trips instead of one.
// Order of pick() is preserved so ties still resolve to the simpler route.
const jobs=[Q("e453166e",a+rf+tl,4,9),Q("4c464f59",a+tl,8,13)];
if(isIn){
...
// the split builders bound every leg with SS, not the user's slippage, so the
// displayed Min must be derived from SS or it understates the enforced floor.
for(const x of["892af013","85f86a90"])jobs.push(Q(x,a+ps,4,8,1,SS));
}
for(const y of await Promise.all(jobs.map(p=>p.catch(()=>null))))pick(y);
if(my!==seq)return;
...
const src=(s===3||s===4)&&r.best.feeBps?`${raw} ${Number(r.best.feeBps)/100}%`:raw;
rate.textContent=isFinite(rx)?`1 ${f.sym} ≈ ${rx>=1?rx.toLocaleString("en-US",{maximumFractionDigits:4}):rx.toPrecision(4)} ${t.sym} · ${src}`:"";
rate.textContent+=isIn?` · Min ${trimAmt(r.amountLimit,t.dec)} ${t.sym}`:` · Max ${trimAmt(r.amountLimit,f.dec)} ${f.sym}`;
const inApprove=isIn?swapAmt:r.msgValue||r.amountLimit;
if(acct)last={callData:r.callData,msgValue:r.msgValue,amountIn:inApprove,from:f,exp:Date.now()+1500000};
render();
...
addr.textContent=short;addr.title=a;
...
amt.addEventListener("input",()=>{setMode("in");fitFont(amt);updateSoon()});
outAmt.addEventListener("input",()=>{setMode("out");fitFont(outAmt);updateSoon()});
slip.addEventListener("input",()=>{last=null;render();updateSoon()});
rc.addEventListener("input",()=>{last=null;sendReady=null;render();updateSoon()});
slip.addEventListener("change",()=>{slip.value=Math.min(10,Math.max(.01,+slip.value||.5))});
...
stat.textContent="Couldn't load token: "+(e.message||e);
sel.value=prev;
}
});
}
flip.onclick=()=>{const a=fromSel.value,b=toSel.value,v=outAmt.value.replace(/,/g,"");fromSel.value=b;toSel.value=a;if(v&&!isNaN(+v))amt.value=v;setMode("in");fitFont(amt);syncDisabled();refreshBalance();update()};
...
const ps=await Promise.all(keys.map(k=>
...
tabSend.textContent=ready?`Send (${ready})`:"Send";
...
const setTab=t=>{
tab=t;last=null;sendReady=null;stat.textContent="";
tabSwap.classList.toggle("on",t==="swap");tabSend.classList.toggle("on",t==="send");
for(const el of[rcvPanel,flip,slipL])el.classList.toggle("hide",t==="send");
dlyL.classList.toggle("hide",t!=="send");
payL.textContent=t==="send"?"Amount":"You pay";
loadPos();
rc.placeholder=t==="send"?"Recipient \u2014 0x or .eth/.(g)wei":"Recipient (optional) \u2014 0x or .eth/.(g)wei";
outAmt.value="";mode="in";
update();
};
dly.onchange=()=>render();
setInterval(()=>{if(account&&!document.hidden)loadPos()},3e4);
...
dly.value=String(os.find(v=>v>=lk)??os[os.length-1])}
...
swap.disabled=true;
try{
const {callData,msgValue,amountIn,from,to}=last;
...
if(!permitted&&await p2Ready(from.addr,account)>=amountIn){try{
stat.textContent="Sign approval...";
const pd=BigInt(Math.floor(Date.now()/1e3)+1800);
cd2=encCalls([await signPermit2(from.addr,account,amountIn,pd),callData]);
permitted=true;
}catch(e){if(isRejection(e)){stat.textContent="";swap.disabled=false;return}}}
if(!permitted&&await canBatch(account)){
batchCalls=[{to:from.addr,data:"0x"+SEL_APPROVE+encAddr(ZROUTER)+encUint(amountIn),value:"0x0"},
