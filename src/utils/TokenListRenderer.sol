...
contract TokenListRenderer {
    ...
    function tokenURI(uint256, TokenList.Token memory t, Extra[] memory extras) public pure returns (string memory) {
        ...
        return _dataURI(
            ...
            string.concat(
                ...
                t.synced
                    ? "Onchain"
                    : t.standard == TokenList.Standard.NATIVE ? "No contract" : "Attested",
    ...
    function _provenanceChip(bool synced, TokenList.Standard standard, string memory theme)
        ...
        returns (string memory)
    {
        // A native asset is not attested because a curator PREFERRED to type its text
        // — it is attested because there is no contract at `address(0)` to read, on
        // any chain, ever. "OWNER ATTESTED" is true but flattens those two situations
        // into one, and readers took it as a warning that someone had chosen the
        // numbers. Say the actual reason; the identity line already carries "NATIVE
        // ASSET", so this does not repeat it.
        string memory text = synced
        ...
        returns (string memory)
    {
        ...
        // Named on the card rather than left as UNVERIFIED. A Tacit asset is listed
        // under `Kind.OTHER`, so the chain line can only say `raw:0`; without this the
        // one row with room to identify it read "TOKEN STANDARD UNVERIFIED", which
        // describes a failure to check rather than a deliberate, attested listing.
        // Named on the card rather than left as UNVERIFIED. These are listed under
