...
contract Bebop {
    ...
    address constant BEBOP_BLEND = 0xbbbbbBB520d69a9775E85b458C58c648259FAD5F; // PMM approval target
        ...
        payable
    {
        ...
        if (tokenIn == address(0)) {
            ...
            // Router API may return JAM or PMM settlement — approve both lazily
            if (allowance(tokenIn, address(this), JAM_BALANCE_MGR) == 0) {
                safeApprove(tokenIn, JAM_BALANCE_MGR, type(uint256).max);
            }
            if (allowance(tokenIn, address(this), BEBOP_BLEND) == 0) {
                safeApprove(tokenIn, BEBOP_BLEND, type(uint256).max);
            }
        }
