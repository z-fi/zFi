...
contract PrecisionPoolLens {
        ...
        returns (uint256 amountOut)
    {
        ...
        uint256 v0 = FixedPointMathLib.fullMulDiv(supply, WAD, p.sqrtPHigh());
        uint256 v1 = FixedPointMathLib.fullMulDiv(supply, p.sqrtPLow(), WAD);
        ...
        uint256 inAfterFee = net - FixedPointMathLib.fullMulDiv(net, p.fee(), FEE_DENOM);
    ...
    function infoFor(address pool, address sender, uint256 probe) public view returns (PoolInfo memory o) {
        ...
        // What a swap of `probe` would actually be charged right now,
        // surcharge included - the number a user should be shown.
        o.effectiveFee = o.fee + p.extraFee(sender, o.token0, probe);
    ...
    function markets(uint256 start, uint256 count, address sender, uint256 probe)
        external
        view
        returns (PoolInfo[] memory)
    {
