// poseidon3 from poseidon-lite@0.3.0 (vendored)
(function(){
    ...
    if (Array.isArray(o)) {
      ...
      for (var key in o) {
        if (o.hasOwnProperty(key)) {
          res[key] = unstringifyBigInts(o[key]);
        }
      }
      return res;
    }
    var byteArray = Uint8Array.from(atob(o), function(c) { return c.charCodeAt(0); });
    var hex = Array.from(byteArray).map(function(x) { return x.toString(16).padStart(2, '0'); }).join('');
  ...
  function poseidon(_inputs, opt, nOuts) {
    if (typeof nOuts === 'undefined') nOuts = 1;
    var inputs = _inputs.map(function(i) { return BigInt(i); });
    if (inputs.length <= 0) throw new Error('poseidon-lite: Not enough inputs');
    if (inputs.length > N_ROUNDS_P.length) throw new Error('poseidon-lite: Too many inputs');
    var t = inputs.length + 1;
    var nRoundsF = N_ROUNDS_F;
    var nRoundsP = N_ROUNDS_P[t - 2];
    var C = opt.C;
    var M = opt.M;
    if (M.length !== t) throw new Error('poseidon-lite: Incorrect M length, expected ' + t + ' got ' + M.length);
    var state = [0n].concat(inputs);
    for (var x = 0; x < nRoundsF + nRoundsP; x++) {
      for (var y = 0; y < state.length; y++) {
    ...
    if (nOuts === 1) return state[0];
    else if (nOuts <= state.length) return state.slice(0, nOuts);
    else throw new Error('poseidon-lite: Invalid number of outputs requested ' + nOuts + ', max ' + state.length);
  }
  ...
  var c = unstringifyBigInts(CONSTANTS);
  window.poseidon3 = function(inputs, nOuts) { return poseidon(inputs, c, nOuts); };
})();
