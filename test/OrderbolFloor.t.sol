...
contract OrderbolTest is Test {
    ...
    function setUp() public {
        ...
        executor = new Orderbol(address(swapboard), address(dutchboard), address(floorboard));
    ...
    function test_nativeFloorBidRejectsAnythingButTheCeiling() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(Orderbol.InputMismatch.selector, END, START));
    ...
    function test_donatedQuoteCannotBecomeSomeonesFloorBid() public {
        ...
        executor.checkpoint(address(quote));
        vm.expectRevert(abi.encodeWithSelector(Orderbol.InputMismatch.selector, END, 0));
        executor.placeFloor(
            address(floorboard), maker, sponsor, address(lot), address(quote), WANT, START, END, 0, 1 hours, 0
        );
    ...
    function test_constructorRejectsARepeatedOrMissingBoard() public {
        vm.expectRevert(Orderbol.BadOrder.selector);
        new Orderbol(address(swapboard), address(swapboard), address(floorboard));

        vm.expectRevert(Orderbol.BadOrder.selector);
        new Orderbol(address(swapboard), address(dutchboard), address(0));
    }
}
