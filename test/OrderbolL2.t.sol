...
/// @dev Orderbol.t.sol run against the L2 variant, bound to a WETH that is NOT
///      the mainnet constant. Everything the original asserts must still hold.
contract OrderbolL2Test is Test {
    ...
    function setUp() public {
        ...
        executor = new OrderbolL2(address(swapboard), address(dutchboard), address(new Floorboard(WETH)), WETH);
    ...
    function test_forcedEthAndDonatedTokensAreNotGiftedToNextMaker() public {
        ...
        executor.checkpoint(address(lot));
        vm.expectRevert(abi.encodeWithSelector(OrderbolL2.InputMismatch.selector, 1e18, 0));
        executor.placeSwapboard(
            address(swapboard),
            address(this),
            address(this),
