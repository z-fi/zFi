...
contract TokenListTest is Test, PostDeployListings {
    ...
    function testLongDescriptionWrapsInsideTheCard() public {
        ...
        // The band's exact y moves with the layout — the block is centred on its line
        // count — so pin the property, not the coordinate: the description is drawn as
        // multiple monospace runs, each within the glyph budget.
        uint256 runs;
        uint256 at;
        while (true) {
