...
import {PrecisionRouteL2} from "../src/pools/PrecisionRouteL2.sol";
...
/// @dev PrecisionPoolMultihop.t.sol with every route executor swapped for the
///      L2 variant. Same fork, same pools, same assertions.
contract PrecisionPoolMultihopL2RouteTest is Test {
    ...
    function test_RouteExecutorDoesEthInMultihopInOneLeg() public {
        PrecisionRouteL2 router = new PrecisionRouteL2(factory, EXEC, WETH);
        ...
        uint256 out = ISnwap(ZROUTER).snwap{value: 1 ether}(
            ...
            abi.encodeCall(PrecisionRouteL2.route, (pools, address(0), WBTC, 1 ether, 0, user))
        );
    ...
    function test_RouteExecutorRunsACheckpointedErc20Route() public {
        ...
        bytes memory call_ = abi.encodeCall(PrecisionRouteL2.route, (pools, WBTC, address(0), amountIn, 0, user));
        uint256 before = user.balance;
        ...
        (bool ok, bytes memory err) = address(router).call(
            abi.encodeCall(PrecisionRouteL2.route, (theirs, WBTC, USDC, amountIn, 0, address(0xBAD)))
        );
        assertFalse(ok, "an uncommitted route spent the funding");
        assertEq(bytes4(err), PrecisionRouteL2.IntentMismatch.selector, "wrong rejection");
    ...
    function test_RouteExecutorRejectsUnknownPools() public {
        ...
        (bool ok, bytes memory err) = address(router).call{value: 1 ether}(
            abi.encodeCall(PrecisionRouteL2.route, (pools, address(0), USDC, 1 ether, 0, user))
        );
        assertFalse(ok, "unknown pool was accepted");
        assertEq(bytes4(err), PrecisionRouteL2.NoPool.selector, "wrong rejection");
    }
    ...
    function test_RouteExecutorRefusesUntrustedCallers() public {
        ...
        assertEq(bytes4(err), PrecisionRouteL2.NotExecutor.selector, "wrong rejection");
    }
    ...
    function test_ZapInFromOneAssetMintsLpSharesInOneTransaction() public {
        ...
        ISnwap(ZROUTER).snwap{value: 2 ether}(
            ...
            abi.encodeCall(PrecisionRouteL2.zapIn, (address(ethUsdc), address(0), 2 ether, 1 ether, 0, user, user))
        );
    ...
    function test_ZapInSweepsOnlyItsOwnLeftoversNotRestingBalances() public {
        ...
        ISnwap(ZROUTER).snwap{value: 2 ether}(
            address(0), 2 ether, user, address(ethUsdc), 0, address(router),
            abi.encodeCall(PrecisionRouteL2.zapIn, (address(ethUsdc), address(0), 2 ether, 1 ether, 0, address(0xDEAD), address(0xDEAD)))
        );
    ...
    function test_NativeEntryIsRefusedWhileAnotherRouteIsOpen() public {
        ...
        address[] memory pools = new address[](1);
        pools[0] = address(ethUsdc);
        bytes memory call_ = abi.encodeCall(PrecisionRouteL2.route, (pools, WBTC, USDC, 1e8, 0, user));
        ...
        // the public executor.
        vm.prank(EXEC);
        vm.expectRevert(PrecisionRouteL2.Reentrancy.selector);
    ...
    function test_ZapInRefusesUnknownPools() public {
        ...
        vm.prank(EXEC);
        (bool ok, bytes memory err) = address(router).call{value: 1 ether}(
            abi.encodeCall(PrecisionRouteL2.zapIn, (address(0xDEAD), address(0), 1 ether, 0, 0, user, user))
        );
        assertFalse(ok);
        assertEq(bytes4(err), PrecisionRouteL2.NoPool.selector);
    }
    ...
    function test_RouteRejectsAPathThatEndsInADifferentTokenThanNamed() public {
        ...
        vm.prank(EXEC);
        vm.expectRevert(PrecisionRouteL2.WrongTokenOut.selector);
