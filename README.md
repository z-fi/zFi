...
Deployment flow: deploy the TWENTY-FOUR generated chunk contracts (`out/zSwap.chunk1..24.creation.txt`), then run `script/build-zSwapNext.mjs` with those addresses: it emits the successor's initcode, `zSwap(dao, previous, [chunk1, ..., chunk24])`, and the calldata for the DAO to pass it to the current tip's `deployNext`. The constructor refuses any other sender for a successor; only a root (`previous` = 0) is deployed directly. The count is not decorative - the constructor reverts `InvalidData` unless all twenty-four are present, non-empty and distinct, and `script/check-zSwap.mjs` fails the build if the source and the Solidity disagree about it. Record the final wrapper address here after deployment.
...
The HTML payload is installed as the **runtime bytecode of twenty-four data contracts** created before the wrapper. The wrapper keeps twenty-four `immutable` pointers (`DATA1`…`DATA24`). At read time, `html()` copies all twenty-four chunks back with `EXTCODECOPY` into one ABI-encoded `string` return — any RPC client decodes it directly.

- **Why multiple data contracts?** EIP-170 caps deployed code at 24,576 bytes. Splitting the page makes that limit apply per chunk instead of to the full dapp. The current payload is 579,753 bytes across 24 data contracts (24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,157 / 24,142 bytes), with 10,071 bytes of 24-chunk headroom.
