...
Deployment flow: deploy the TWENTY-EIGHT generated chunk contracts (`out/zSwap.chunk1..28.creation.txt`), then run `script/build-zSwapNext.mjs` with those addresses: it emits the successor's initcode, `zSwap(dao, previous, [chunk1, ..., chunk28])`, and the calldata for the DAO to pass it to the current tip's `deployNext`. The constructor refuses any other sender for a successor; only a root (`previous` = 0) is deployed directly. The count is not decorative - the constructor reverts `InvalidData` unless all twenty-eight are present, non-empty and distinct, and `script/check-zSwap.mjs` fails the build if the source and the Solidity disagree about it. Record the final wrapper address here after deployment.

Lineage so far: v0.1 root [`0x00000095643CFfA7D9fae407a84dfCB6406456c6`](https://etherscan.io/address/0x00000095643CFfA7D9fae407a84dfCB6406456c6), v0.2 tip [`0xe686952842627A2cf81DF42CCaD54ef98046DB8D`](https://etherscan.io/address/0xe686952842627A2cf81DF42CCaD54ef98046DB8D). [`zSwapResolver`](https://etherscan.io/address/0x000000E7DAD6128683D1fb415e80B30c23dAb7AC) serves the newest version once it is 3 days old, and [`zSwapFlags`](https://etherscan.io/address/0x0000008a1a3C78440d0a28EBB4bb5526abA91d45) switches the solver lanes on and off. `zEndpoints` (`0x00000051F365d898132f4ebF345Cd3968e02F288`, live since block 26005405) curates every off-chain endpoint the page reads that is not an Ethereum node - L2 read nodes, wide-history log nodes, the private pool's relay, Bitcoin APIs and the WalletConnect relay - each merged ahead of the values the page was built with, which remain the fallback.
...
The HTML payload is installed as the **runtime bytecode of twenty-eight data contracts** created before the wrapper. The wrapper keeps twenty-eight `immutable` pointers (`DATA1`…`DATA28`). At read time, `html()` copies all twenty-eight chunks back with `EXTCODECOPY` into one ABI-encoded `string` return — any RPC client decodes it directly.

- **Why multiple data contracts?** EIP-170 caps deployed code at 24,576 bytes. Splitting the page makes that limit apply per chunk instead of to the full dapp. The current payload is 675,751 bytes across 28 data contracts (24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,134 / 24,133 bytes), with 12,377 bytes of 28-chunk headroom.
...
- Quotes via [zQuoter](https://etherscan.io/address/0x000000bd2db80567c23e353ca95a251c573cbf9b) across Uniswap V2/V3/V4, Sushi, Curve and Lido.
