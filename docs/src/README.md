...
Deployment flow: deploy the EIGHTEEN generated chunk contracts (`out/zSwap.chunk1..18.creation.txt`), then deploy `zSwap(dao, previous, chunk1, ..., chunk18)` with those addresses. The count is not decorative - the constructor reverts `InvalidData` unless all eighteen are present, non-empty and distinct, and `script/check-zSwap.mjs` fails the build if the source and the Solidity disagree about it. Record the final wrapper address here after deployment.
...
The HTML payload is installed as the **runtime bytecode of eighteen data contracts** created before the wrapper. The wrapper keeps eighteen `immutable` pointers (`DATA1`…`DATA18`). At read time, `html()` copies all eighteen chunks back with `EXTCODECOPY` into one ABI-encoded `string` return — any RPC client decodes it directly.

- **Why multiple data contracts?** EIP-170 caps deployed code at 24,576 bytes. Splitting the page makes that limit apply per chunk instead of to the full dapp. The current payload is 428,032 bytes across 18 data contracts (23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,780 / 23,772 bytes), with 14,336 bytes of 18-chunk headroom.
