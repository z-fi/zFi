...
Deployment flow: deploy the TWENTY-FIVE generated chunk contracts (`out/zSwap.chunk1..25.creation.txt`), then run `script/build-zSwapNext.mjs` with those addresses: it emits the successor's initcode, `zSwap(dao, previous, [chunk1, ..., chunk25])`, and the calldata for the DAO to pass it to the current tip's `deployNext`. The constructor refuses any other sender for a successor; only a root (`previous` = 0) is deployed directly. The count is not decorative - the constructor reverts `InvalidData` unless all twenty-five are present, non-empty and distinct, and `script/check-zSwap.mjs` fails the build if the source and the Solidity disagree about it. Record the final wrapper address here after deployment.
...
The HTML payload is installed as the **runtime bytecode of twenty-five data contracts** created before the wrapper. The wrapper keeps twenty-five `immutable` pointers (`DATA1`…`DATA25`). At read time, `html()` copies all twenty-five chunks back with `EXTCODECOPY` into one ABI-encoded `string` return — any RPC client decodes it directly.

- **Why multiple data contracts?** EIP-170 caps deployed code at 24,576 bytes. Splitting the page makes that limit apply per chunk instead of to the full dapp. The current payload is 597,522 bytes across 25 data contracts (23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,901 / 23,898 bytes), with 16,878 bytes of 25-chunk headroom.
