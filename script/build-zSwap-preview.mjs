...
(function () {
  ...
  function ethCall(tx) {
    ...
    // The curated list, so the picker carries a COLLECTION and the NFT paths
    // are reachable at all. Without this the page falls back to its hardcoded
    // tokens, which are all fungible, and the Token ID field can never appear.
    if (sel === "df7ca268") return "0x" + u256(32) + u256(2) + u256(0) + u256(1);
    if (sel === "74e18e96") {
      var which = Number(BigInt("0x" + data.slice(8, 72)));
      return encStr(which === 0
        ? '{"k":"eip155","p":"ERC-20","a":"' + A.USDC + '","d":6,"s":"USDC"}'
        : '{"k":"eip155","p":"ERC-721","a":"' + NFTCOL + '","d":0,"s":"ZORB"}');
    }
  ...
  var NFTCOL = "0x00000000000000000000000000000000000000ff";
